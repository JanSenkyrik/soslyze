...
rhel_7_tasks() {
    ...
    SUB_PLATFORM=$(grep -E "^hostname|^baseurl|^port" ./etc/rhsm/rhsm.conf)
    ...
    SUB_URLS=$(grep -E "enabled=1|enabled= 1|enabled = 1|enabled =1" ./etc/yum.repos.d/redhat.repo -B 12 | grep -E "\[|enabled[^_]|baseurl")
...
rhel_8_tasks() {
    ...
    # Subscription Management
    SUB_PLATFORM=$(grep -E "^hostname|^baseurl|^port|^repo_ca_cert|^ca_cert_dir" ./etc/rhsm/rhsm.conf)
    SUB_PROXY=$(grep -E ^proxy ./etc/rhsm/rhsm.conf)
    SUB_PROXY_ENV=$(grep -i proxy ./environment)
    SUB_SCA=$(for cert in `ls ./etc/pki/entitlement/`; do rct cat-cert ./etc/pki/entitlement/$cert | grep content_access; done)
    SUB_CONSUMED=$(cat ./sos_commands/subscription_manager/subscription-manager_list_--consumed | grep -E "Subscription Name|Subskriptionsname|Nom de l'abonnement|SKU|Starts|Ends|Pool ID|Status Details")
...
insights_tasks() {
    ...
    INSIGHTS_CONFIG=$(grep -Ev '^\s*(#|$)' ./etc/insights-client/insights-client.conf)
...
sat_6_tasks() {
    SAT_RELEASE=$(grep -E "satellite-6|capsule" ./installed-rpms)
    SAT_HEALTH_CHECK=$(cat ./sos_commands/foreman/hammer_ping)
    SAT_CERTS=$(grep -Ei 'server_key|server_cert_req|server_ca_cert|server_cert' ./etc/foreman-installer/scenarios.d/satellite-answers.yaml)
    SAT_HIERA=$(grep -Ev '^\s*(#|$)' ./etc/foreman-installer/custom-hiera.yaml)
    ...
    SAT_TUN_PUMA_WORKERS=$(grep -E "PUMA_THREADS|PUMA_WORKERS" ./etc/systemd/system/foreman.service.d/installer.conf)
    ...
    SAT_TUN_PUPPET=$(grep -E "^JAVA_ARGS=" ./etc/sysconfig/puppetserver)
    SAT_TUN_PGSQL=$(grep -E 'max_connections|shared_buffers|work_mem|autovacuum_vacuum_cost_limit' ./var/lib/pgsql/data/postgresql.conf)
    ...
    SAT_SETTINGS=$(head -1 ./sos_commands/foreman/foreman_settings_table && grep -E 'http_proxy|allow_auto_inventory_upload|destroy_vm_on_host_delete|foreman_tasks_proxy_batch_trigger|foreman_tasks_proxy_batch_size|remote_execution_ssh_user|remote_execution_effective_user|foreman_ansible_proxy_batch_size|content_default_http_proxy|subscription_connection_enabled|default_download_policy|default_redhat_download_policy' ./sos_commands/foreman/foreman_settings_table)
    SAT_ANSIBLE=$(grep -E "ANSIBLE_ROLES_PATH|ANSIBLE_COLLECTIONS_PATHS|ANSIBLE_SSH_ARGS" ./etc/foreman-proxy/ansible.env)
